...
function MyJobsView() {
  const { myJobs, loadMyJobs } = useApp();
  const [loading, setLoading] = useState(false);

  useEffect(() => {
    setLoading(true);
    loadMyJobs().finally(() => setLoading(false));
  }, []); // eslint-disable-line react-hooks/exhaustive-deps

  const applied = myJobs.filter((j) => j.status === "pending");
  const ongoing = myJobs.filter((j) => j.status === "ongoing");
  ...
  if (myJobs.length === 0) {
    return (
      <div className="card p-16 text-center">
        ...
        <p className="text-sm text-gray-500 dark:text-gray-400">Jobs you apply to or get assigned will appear here.</p>
      </div>
    );
  }

  const Section = ({ title, icon, jobs }) => jobs.length === 0 ? null : (
    <div className="mb-6">
      <h3 className="text-sm font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-3 flex items-center gap-2">
        ...
        <span className="ml-auto text-xs font-normal normal-case">{jobs.length} job{jobs.length !== 1 ? "s" : ""}</span>
      </h3>
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        {jobs.map((job) => <JobCard key={job.id} job={job} showActions={false} />)}
      </div>
    </div>
  );
  ...
  return (
    <div>
      <Section title="Applied — Awaiting Response" icon="📋" jobs={applied} />
      <Section title="Ongoing — You're Assigned" icon="🔄" jobs={ongoing} />
      <Section title="Completed" icon="✅" jobs={completed} />
    </div>
  );
}
